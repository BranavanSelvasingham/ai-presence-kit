...
assert.match(output, /assistant-lifecycle:submit->thinking\+\d+ms phase=before-output attention=response face=thinking/);
assert.match(output, /assistant-lifecycle:stream-open->waiting\+\d+ms phase=before-output attention=response face=listening/);
assert.match(output, /assistant-lifecycle:token->streaming\+\d+ms phase=output attention=audience face=speaking/);
...
assert.match(output, /assistant-lifecycle:summary traceSummary=entries:\d+ .*streamOpenMs=\d+ms firstTokenMs=\d+ms .*firstOutput=token interruptMs=none interrupted=false leadMs=\d+ms finalState=ready hasOutput=true complete=true/);
...
assert.match(output, /assistant-lifecycle:[^\n]+trace=complete decisions=6 safe=true warnings=0 transition=[^:\s]+:[^+\s]+\+0ms transitionReads=6\/6 reads=state,transitionEvent,transitionAgeMs/);
...
assert.match(output, /assistant-lifecycle:stream-open->[^\n]+transition=thinking:stream-open\+0ms transitionReads=6\/6/);
assert.match(output, /assistant-lifecycle:token->[^\n]+transition=waiting:token\+0ms transitionReads=6\/6/);
