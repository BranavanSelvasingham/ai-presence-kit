...
`npm run browser:smoke` is included in `release:preflight` for publish readiness. A missing local Chrome executable is a release visibility gap; set `CHROME_PATH` to a local Chrome executable and rerun preflight.
