(function initPresenceFace(globalScope) {
  ...
  function controlsForState(state, detail, profile) {
    const stateName = state || PresenceState.IDLE || "idle";
    const base = {
      gaze: { target: "user", x: 0, y: 0, focus: 0.56 },
      blink: { openness: 1, cadenceMs: profile.blinkCadenceMs, pulse: false },
      brows: { lift: 0, pinch: 0, asymmetry: 0 },
      mouth: { shape: "rest", openness: 0, activity: 0, tension: 0 },
      posture: { lean: 0, turn: 0, energy: 0.2, recovery: 0 },
      motion: { energy: 0.2, drift: profile.drift, anticipation: 0, recovery: 0, settleMs: profile.settleMs },
    };
    ...
    switch (stateName) {
      ...
      case "user-typing":
        return {
          gaze: { target: "input", x: -0.18, y: 0.18, focus: 0.7 },
          blink: { openness: 1, cadenceMs: 4400, pulse: false },
          brows: { lift: 0.08, pinch: 0.06, asymmetry: 0 },
          mouth: { shape: "listening", openness: 0.03, activity: 0.05, tension: 0.08 },
          posture: { lean: 0.1, turn: -0.04, energy: 0.38, recovery: 0 },
          motion: { energy: 0.34, drift: profile.drift + 0.04, anticipation: 0.18, recovery: 0, settleMs: profile.settleMs },
        };

      case PresenceState.READING:
      case "reading":
        return {
          gaze: { target: detail.question ? "question" : "content", x: -0.2, y: 0.28, focus: 0.76 },
          ...
          mouth: { shape: detail.question ? "curious" : "held", openness: 0.04, activity: 0.08, tension: detail.revision ? 0.32 : 0.12 },
          posture: { lean: 0.14, turn: -0.06, energy: 0.42, recovery: 0 },
          motion: { energy: 0.34, drift: profile.drift + 0.02, anticipation: 0.22, recovery: 0, settleMs: profile.settleMs },
        };

      case PresenceState.WAITING:
      case "waiting":
        return {
          gaze: { target: "response-origin", x: -0.08, y: -0.04, focus: 0.66 },
          blink: { openness: 0.9, cadenceMs: 3400, pulse: false },
          brows: { lift: 0.02, pinch: 0.28, asymmetry: 0.04 },
          mouth: { shape: "preparing", openness: 0.03, activity: 0.16, tension: 0.34 },
          posture: { lean: 0.24, turn: 0, energy: 0.5, recovery: 0 },
          motion: { energy: 0.46, drift: profile.drift + 0.08, anticipation: 0.62, recovery: 0, settleMs: 120 },
        };

      case PresenceState.THINKING:
      case "thinking":
        return {
          gaze: { target: "middle-distance", x: 0.16, y: -0.02, focus: 0.58 },
          blink: { openness: 0.82, cadenceMs: 3800, pulse: false },
          brows: { lift: -0.04, pinch: 0.36, asymmetry: 0.08 },
          mouth: { shape: "pressed", openness: 0.02, activity: 0.12, tension: 0.42 },
          posture: { lean: 0.18, turn: 0.05, energy: 0.48, recovery: 0 },
          motion: { energy: 0.42, drift: profile.drift + 0.05, anticipation: 0.52, recovery: 0, settleMs: 140 },
        };

      case PresenceState.STREAMING:
      case "streaming":
        return {
          gaze: { target: "audience", x: 0, y: 0, focus: 0.74 },
          blink: { openness: 0.98, cadenceMs: 6800, pulse: false },
          brows: { lift: 0.08, pinch: 0.08, asymmetry: 0 },
          mouth: { shape: "speaking", openness: 0.34, activity: 0.82, tension: 0.06 },
          posture: { lean: 0.14, turn: 0, energy: 0.72, recovery: 0 },
          motion: { energy: 0.78, drift: profile.drift + 0.1, anticipation: 0.12, recovery: 0, settleMs: 90 },
        };

      case PresenceState.SPEAKING:
      case "speaking":
        return {
          gaze: { target: "audience", x: 0, y: -0.02, focus: 0.78 },
          blink: { openness: 0.98, cadenceMs: 7200, pulse: false },
          brows: { lift: 0.12, pinch: 0.04, asymmetry: 0 },
          mouth: { shape: "speaking", openness: 0.42, activity: 1, tension: 0.04 },
          posture: { lean: 0.12, turn: 0, energy: 0.78, recovery: 0 },
          motion: { energy: 0.86, drift: profile.drift + 0.12, anticipation: 0, recovery: 0, settleMs: 80 },
        };

      case PresenceState.INTERRUPTED:
      case "interrupted":
        return {
          gaze: { target: "user", x: -0.26, y: -0.08, focus: 0.88 },
          blink: { openness: 0.72, cadenceMs: 900, pulse: true },
          brows: { lift: -0.12, pinch: 0.54, asymmetry: 0.34 },
          mouth: { shape: "held", openness: 0.07, activity: 0.04, tension: 0.72 },
          posture: { lean: -0.22, turn: -0.08, energy: 0.62, recovery: 0.16 },
          motion: { energy: 0.54, drift: profile.drift + 0.04, anticipation: 0, recovery: 1, settleMs: 90 },
        };

      case PresenceState.READY:
      case "ready":
        return {
          gaze: { target: "user", x: 0, y: 0, focus: 0.68 },
          blink: { openness: 1, cadenceMs: 4600, pulse: false },
          brows: { lift: 0.1, pinch: 0, asymmetry: 0 },
          mouth: { shape: "soft-smile", openness: 0.12, activity: 0.1, tension: 0 },
          posture: { lean: 0.02, turn: 0, energy: 0.3, recovery: 0 },
          motion: { energy: 0.24, drift: profile.drift, anticipation: 0, recovery: 0, settleMs: profile.settleMs + 40 },
        };

      case PresenceState.ERROR:
      case "error":
        return {
          gaze: { target: "status", x: 0, y: 0.18, focus: 0.8 },
          blink: { openness: 0.86, cadenceMs: 3000, pulse: false },
          brows: { lift: -0.08, pinch: 0.5, asymmetry: 0.08 },
          mouth: { shape: "downturned", openness: 0.03, activity: 0, tension: 0.66 },
          posture: { lean: -0.12, turn: 0, energy: 0.42, recovery: 0 },
          motion: { energy: 0.18, drift: profile.drift * 0.6, anticipation: 0, recovery: 0.24, settleMs: profile.settleMs + 80 },
        };
  ...
  function applyHistoryAdjustments(controls, snapshot, history, now) {
    const ageMs = stateAgeMs(snapshot, history, now);
    const recentlyInterrupted = includesRecentState(snapshot, history, PresenceState.INTERRUPTED || "interrupted", 2400, now);
    const recentlySpoke = includesRecentState(snapshot, history, PresenceState.STREAMING || "streaming", 1800, now)
    ...
    if (snapshot.state === PresenceState.READY || snapshot.state === "ready") {
  ...
  function faceControlsForPresence(snapshotOrState, options = {}) {
    ...
    const controls = controlsForState(snapshot.state, snapshot.detail, profile);

    controls.expression = faceExpressionForPresence(snapshotOrState, options);
    return freezeControls(applyHistoryAdjustments(controls, snapshot, history, now));
  }
